...
<script>
	function pedirPrestamo(id) {
		if (confirm("¿Desea pedir un préstamo?")) {
			location.href = "PrestamosController?operacion=nuevo&idcliente="
					+ id;
		}
	}
	function modificarPrestamo(id) {
		if (confirm("¿Desea modificar el préstamo?")) {
			location.href = "PrestamosController?operacion=editar&idprestamo="
					+ id;
		}
	}
	function eliminarPrestamo(idprestamo, idcliente) {
		if (confirm("¿Desea eliminar el préstamo?")) {
			location.href = "PrestamosController?operacion=eliminar&idprestamo="
					+ idprestamo + "&idcliente=" + idcliente;
		}
	}
</script>
</head>
<body data-bs-theme="dark">
	...
	<div class="container">
		<br> <a href="javascript:pedirPrestamo(<%=idcliente%>)"
			class="btn btn-success"> Nuevo prestamo </a> <a
			href="<%=url%>ClientesController?operacion=listar"
			class="btn btn-success"> Volver a los clientes </a> <br>
		<p></p>
		<table id="tabla" class="table table-bordered">
			...
			<tbody>
				...
				<tr>
					...
					<td><a
						href="javascript:modificarPrestamo(<%=prestamo.getIdprestamo()%>)"
						class="btn btn-warning"> Editar </a> <a
						href="javascript:eliminarPrestamo(<%=prestamo.getIdprestamo()%>, <%=prestamo.getIdcliente()%>)"
